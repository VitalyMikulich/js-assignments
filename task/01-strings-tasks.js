...
function getRectangleString(width, height) {
    ...
    for(let j = 1; j <= height; j++) {
        for(let i = 1; i <= width; i++) {
            ...
            if(i == width && j == 1) str += '┐';
            if(i == 1 && j == height) str += '└';
            if(i == width && j == height) str += '┘';
            if((j == 1 || j == height) && (i > 1 && i < width)) str += '─';
            if((i == 1 || i == width) && (j > 1 && j < height)) str += '│';
            if((i > 1 && i < width) && (j > 1 && j < height)) str += ' ';
            if(i == width) str += '\n';
        }
    }
...
function encodeToRot13(str) {
    // throw new Error('Not implemented');
    let start = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    let end   = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    return str.split('').map(el => {
        let num = start.indexOf(el);
        if(num == -1) return el;
        return end[num]; 
    }).join('');
}
